 ...
 */
package io.jshift.generator.api.support;
...
import static io.jshift.kit.config.image.build.OpenShiftBuildStrategy.SourceStrategy.name;
import static io.jshift.kit.config.image.build.OpenShiftBuildStrategy.SourceStrategy.kind;
import static io.jshift.kit.config.image.build.OpenShiftBuildStrategy.SourceStrategy.namespace;
...
public class BaseGeneratorTest {
    ...
    public void fromAsConfigured() {
        ...
        projectProps.put("jshift.generator.from","propFrom");
    ...
    public void addFromIstagModeWithSelector() {
        Properties props = new Properties();
        props.put("jshift.generator.fromMode","istag");
    ...
    public void getRegistry() {
        Properties props = new Properties();
        props.put("jshift.generator.registry", "jshift.io");
        setupContextKubernetes(props, null, null);

        BaseGenerator generator = createGenerator(null);
        assertEquals("jshift.io", generator.getRegistry());
    }
    ...
    private void setupNameContext(String propertyName, final String configName) {
        ...
        if (propertyName != null) {
            props.put("jshift.generator.name", propertyName);
        }
        new Expectations() {{
            ctx.getProject(); result = project;
