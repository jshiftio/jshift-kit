...
import io.jshift.kit.build.service.docker.helper.DockerFileUtil;
...
public class OpenshiftBuildService implements BuildService {
    ...
    public void build(ImageConfiguration imageConfig) throws JshiftServiceException {
        ...
        try {
        ...
        } catch (JshiftServiceException e) {
            throw e;
        } catch (Exception ex) {
            ...
            if (ex != null && ex.getCause() instanceof IOException) {
                ...
                throw new JshiftServiceException("Unable to build the image using the OpenShift build service", ex);
            }
        }
    }

    protected File createBuildArchive(ImageConfiguration imageConfig) throws JshiftServiceException {
    ...
    private ArchiverCustomizer getS2ICustomizer(ImageConfiguration imageConfiguration) throws JshiftServiceException {
        try {
            ...
            throw new JshiftServiceException("Unable to add environment variables to the S2I build archive", e);
        }
    }
    ...
    private String extractBaseFromDockerfile(BuildConfiguration buildConfig, io.jshift.kit.build.service.docker.BuildService.BuildContext buildContext) {
        String fromImage;
        try {
    ...
    private void logBuildFailure(OpenShiftClient client, String buildName) throws JshiftServiceException {
        try {
            ...
            for(Build build : builds) {
                if(build.getMetadata().getName().contains(buildName)) {
                    ...
                    throw new JshiftServiceException("Unable to build the image using the OpenShift build service", new KubernetesClientException(build.getStatus().getReason() + " " + build.getStatus().getMessage()));
                }
            }
