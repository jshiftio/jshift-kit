...
public class DefaultControllerEnricher extends BaseEnricher {
    ...
    private Long getOpenshiftDeployTimeoutInSeconds(Long defaultValue) {
        if (getContext().getProperty("jshift.openshift.deployTimeoutSeconds") != null) {
            return Long.parseLong(getContext().getProperty("jshift.openshift.deployTimeoutSeconds").toString());
        } else {
            return defaultValue;
        }
    }
